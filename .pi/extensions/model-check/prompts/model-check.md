...
请完成：
1. 用当前领域模型解释 Given：列出应存在的对象实例、关系和业务事实；若存在 FM 语义，则映射到 Contract、Context、Role、Thing、Evidence。
2. 用当前领域模型解释 When：指出发生的业务动作，并判断它应落在哪个 Evidence、Fulfillment Request 或 Fulfillment Confirmation。
3. 用当前领域模型解释 Then：列出应产生的 Confirmation、下游 Evidence、属性、关系或业务事实变化。
4. 检查模型问题：概念缺失、关系错置、业务规则遗漏、命名歧义、必要 Evidence 缺失、跨 Context 桥接不合法。
