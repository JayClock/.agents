...
- Business rules belong in `precondition`, `calculationRule`, Domain Role, evidence attributes, and node/edge notes where needed.
...
For a new/initial model, return an add-only `changes` payload. Put every generated node in `changes.addNodes` and every generated edge in `changes.addEdges`; keep update/delete arrays empty. This makes the first response use the same transport shape as later incremental updates:
    ...
    "addNodes": [
      ...
      },
      {
        ...
        "data": {
          ...
          "attributes": [
            { "name": "startedAt", "label": "开始时间", "valueType": "DateTime", "required": true, "meaning": "交付申请开始的业务时间" },
            { "name": "expiredAt", "label": "失效时间", "valueType": "DateTime", "required": true, "meaning": "交付申请的业务失效时间" }
          ]
        }
      },
      {
        ...
        "data": {
          ...
          "attributes": [
            { "name": "confirmedAt", "label": "确认时间", "valueType": "DateTime", "required": true, "meaning": "完成交付确认的业务时间" }
          ]
        }
      }
    ],
...
Use stable AI-generated ids with explicit prefixes: node ids must start with `node-`, such as `node-1`, and edge ids must start with `edge-`, such as `edge-1`. These prefixes distinguish newly generated model elements from persisted production records. Node ids must be unique across the full graph or across `changes.addNodes` for an initial add-only response; node `data.name` values must be non-empty and unique; edge ids must be unique across the full graph or across `changes.addEdges` for an initial add-only response. Every edge endpoint must reference a node id in the same model/add-only payload unless the caller provided an existing model with those ids.

Each React Flow edge in `edges` or `changes.addEdges` is a scalar 1:1 relation: `source` is exactly one node id and `target` is exactly one node id. Do not use arrays, comma-separated ids, wildcards, or custom endpoint payloads to express one-to-many. When a Contract has multiple Fulfillment Requests, return multiple independent edge objects that reuse the Contract as `source` and point to one Fulfillment Request each.

Put endpoint cardinality display text in `edge.data.sourceRelation` and `edge.data.targetRelation`. Because each edge is always 1:1, both values must be `"1"`. A React Flow custom edge may read these values and render them near the source and target endpoints with `EdgeLabelRenderer`. Keep `type: "smoothstep"` for portable output. Do not use custom edge `type` values unless the user or caller provides an explicit renderer contract outside the FM payload.
...
Do not use custom edge `type` values such as `relationship`, `role-play`, or `cross-context-association` unless the target React Flow app has registered matching `edgeTypes` outside the FM payload. When portability matters, keep the built-in path `type` and express visual differences with `markerEnd`, `style`, and `data.sourceRelation` / `data.targetRelation`.

For an update to an existing model, keep returning only `changes` by default after locally validating the merged full graph:
...
Complete graph example for a medium-sized model when the caller explicitly asks for the full/final model:

This example shows multiple bounded contexts, one Contract branching to independent Fulfillment Request nodes, a reverse refund flow, same-context Other Evidence, and Evidence As Role cross-context bridges. Each edge is still a single 1:1 React Flow edge. If this is the first/default response rather than an explicitly requested full model, wrap the same node list in `changes.addNodes` and the same edge list in `changes.addEdges`, with update/delete arrays empty.
...
Use `changes` as the default transport for both initial responses and updates. For an initial/new model, return all generated elements as `changes.addNodes` and `changes.addEdges`, with update/delete arrays empty, and validate the add-only payload with `self_check_fm_graph.py`. For updates, validate against the merged full graph before returning success: generate the diff, apply it locally to the provided base graph, run the full graph self-check, and only then return the diff. Within `changes`, update/delete arrays must target existing ids, add arrays must introduce new ids, and the same id must not repeat across add/update/delete arrays for the same graph element type. Return full `nodes` and `edges` only when the caller explicitly asks for the complete/full/final model; for complete updated models, return the validated merged full graph instead of the diff.

Do not include non-rendered diagnostic payloads in the returned graph. If assumptions or manual review reminders are needed, incorporate them into appropriate node/edge `data.notes` fields or state them outside the JSON payload.
