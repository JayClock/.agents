---
name: markdown-html
description: "Convert Markdown to styled standalone HTML with WeChat-compatible themes, syntax highlighting, math, PlantUML, footnotes, alerts, infographics, and optional bottom citations for external links. Use when the user asks for Markdown to HTML, md 转 html, styled HTML output, 微信排版, or 微信外链转底部引用."
version: 1.56.1
metadata:
  openclaw:
...
# Markdown HTML: Styled Publishing Output
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.agents/skills/markdown-html/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/agents/markdown-html/EXTEND.md` | XDG |
| 3 | `$HOME/.agents/skills/markdown-html/EXTEND.md` | User home |
...
If CJK content detected AND the `write` skill can format Markdown:
...
**If user agrees**: Use the `write` skill's Markdown Formatting Mode to format the file, then use formatted file as input.
...
3. A WeChat publishing EXTEND.md `default_theme` (cross-skill fallback)
...
Read `$HOME/.agents/skills/post-to-wechat/EXTEND.md` if it exists and look for a `default_theme:` line. Use the value if present; otherwise fall through.
