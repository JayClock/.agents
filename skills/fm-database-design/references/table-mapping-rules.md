...
1. Node independence: map each business-lifecycle node kind to its own physical table. Do not collapse Contract, Request, Confirmation, and Other Evidence into one status-driven wide table.
...
3. Time-driven flow: preserve lifecycle timestamps as first-class columns because time drives control flow, SLA checks, audit, and reconciliation.
...
- Contract: create the main contract table, such as `subscription_contract`. Store static transaction commitments, contracting parties, target Thing, signed time, agreed amount, duration, quantity, and terms.
- Fulfillment Request: create one request ledger table per action, such as `payment_request`, `shipment_request`, or `refund_request`. Store who initiated the action, when it started, when it expires, target contract, requested amount or quantity, and request-specific parameters.
- Fulfillment Confirmation: create one confirmation receipt table per paired result, such as `payment_confirmation`, `shipment_confirmation`, or `refund_confirmation`. Store the upstream request id, confirmed time, actual result, actual amount or quantity, and external receipt identifiers when relevant.
- Other Evidence: create a concrete business-document table, such as `revenue_recognition`, `electronic_invoice`, `warehouse_receipt`, or `delivery_note`. Store the producing confirmation id, created time, document number, amount, tax, and document-specific facts.
...
- Contract tables require `id`, `signed_at`, participant foreign keys, Thing foreign key when relevant, and static commitments such as `contract_amount`, `currency`, `quantity`, `term_started_at`, or `term_expired_at`.
- RFP and Proposal tables require `id`, `started_at`, and `expired_at`.
- Fulfillment Request tables require `id`, `contract_id` or an upstream evidence id, `started_at`, `expired_at`, initiating Party Role reference, request payload columns, and idempotency key when commands may be retried.
- Fulfillment Confirmation tables require `id`, `request_id`, `confirmed_at`, confirming Party Role reference, result columns, and actual amount/quantity/time fields.
- Other Evidence tables require `id`, producing evidence foreign key, `created_at`, document number or natural key when present, and document-specific facts.
- Cross-context handoff tables or outbox records require source evidence id, event type, occurred time, payload reference, delivery status, and downstream correlation id when available.
...
- Confirmation tables must reference the paired Request table with `request_id`.
...
2. For each table, state the source FM node, purpose, key columns, required timestamps, foreign keys, uniqueness/idempotency constraints, and append-only notes.
