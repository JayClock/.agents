...
Design physical database tables from Fulfillment Modeling (FM) without collapsing business evidence into mutable status tables. Treat the FM graph as the source of truth for table boundaries, business time attributes, foreign keys, and append-only write behavior.

## Workflow

1. Start from an existing FM graph when provided. If the user gives only requirements, first identify the minimum FM elements needed for storage design: Contract, Request and/or Confirmation evidence, Other Evidence, Participant, Thing, Context, and cross-context bridges.
...
5. Carry over business time attributes only when they appear in the FM graph or domain requirements; do not invent mandatory time columns.
...
7. Translate FM edges that appear in the graph into foreign keys and traceability constraints, such as Request -> Contract, Confirmation -> Request, Other Evidence -> producing Confirmation, and cross-context handoffs through immutable evidence plus outbox/inbox or bridge records.
...
- Purpose.
- Key columns and business time columns present in the FM graph or requirements.
